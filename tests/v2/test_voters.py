...
from utils.dummy import vote2, new_vote2, vote_keys2, create_account, user_login, voters_office_value2, \
    voters_candidate_value2, voters_createdBy_value2
...
class TestVote(BaseTest):
    ...
    def test_office_value(self):
        ...
        response = self.client.post(
            '/api/v2/vote', data=json.dumps(voters_office_value2), content_type='application/json',
    ...
    def test_candidate_value(self):
        ...
        response = self.client.post(
            '/api/v2/vote', data=json.dumps(voters_candidate_value2), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
        self.assertEqual(result['message'], 'only positive integer is accepted')
        assert response.status_code == 400
    ...
    def test_createdBy_value(self):
        ...
        response = self.client.post(
            '/api/v2/vote', data=json.dumps(voters_createdBy_value2), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
        self.assertEqual(result['message'], 'only positive integer is accepted')
        assert response.status_code == 400
