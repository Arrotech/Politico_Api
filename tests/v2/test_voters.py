...
class TestVote(BaseTest):
	...
	def test_vote(self):
		...
		response = self.client.post(
			'/api/v2/auth/voters', data=json.dumps(new_vote), content_type='application/json')
	...
	def test_vote_keys(self):
		...
		response = self.client.post(
			'/api/v2/auth/voters', data=json.dumps(vote_keys), content_type='application/json')
		result = json.loads(response.data.decode())
	...
	def test_office_value(self):
		...
		response = self.client.post(
			'/api/v2/auth/voters', data=json.dumps(voters_office_value), content_type='application/json')
		result = json.loads(response.data.decode())
	...
	def test_candidate_value(self):
		...
		response = self.client.post(
			'/api/v2/auth/voters', data=json.dumps(voters_candidate_value), content_type='application/json')
		result = json.loads(response.data.decode())
		self.assertEqual(result['message'], 'input is in wrong format')
		assert response.status_code == 400
	...
	def test_createdBy_value(self):
		...
		response = self.client.post(
			'/api/v2/auth/voters', data=json.dumps(voters_createdBy_value), content_type='application/json')
		result = json.loads(response.data.decode())
		self.assertEqual(result['message'], 'input is in wrong format')
		assert response.status_code == 400