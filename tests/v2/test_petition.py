...
from utils.dummy import new_petition, petition_keys2, petition_office_value, petitioner_value, \
    create_account, user_login
...
class TestPetitions(BaseTest):
    ...
    def test_petition(self):
        ...
        response = self.client.post(
            ...
            '/api/v2/petitions', data=json.dumps(new_petition), content_type='application/json',
    ...
    def test_office_value(self):
        ...
        response = self.client.post(
            '/api/v2/petitions', data=json.dumps(petition_office_value), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_petitioner_value(self):
        ...
        response = self.client.post(
            ...
            '/api/v2/petitions', data=json.dumps(petitioner_value), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
        self.assertEqual(result['message'], 'input is in wrong format')
        assert response.status_code == 400
    ...
    def test_petition_keys(self):
        ...
        response = self.client.post(
            '/api/v2/petitions', data=json.dumps(petition_keys2), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
