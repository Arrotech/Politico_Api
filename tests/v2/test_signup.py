...
from utils.dummy import role_value2, wrong_role_value, edit_account, password_length, empty_password, create_account, \
    user_login2, account_keys, email_value, passport_value, new_account, phone_value, firstname_value, lastname_value, \
    role_value, user_login, unregistered_user, email_exists, phone_exists, passport_exists
...
class TestUsersAccount(BaseTest):
    ...
    def test_empty_password(self):
        ...
        response = self.client.post(
            '/api/v2/auth/signup', data=json.dumps(empty_password), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_password_length(self):
        ...
        response = self.client.post(
            '/api/v2/auth/signup', data=json.dumps(password_length), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_login_wrong_password(self):
        ...
        response = self.client.post(
            '/api/v2/auth/login', data=json.dumps(user_login2), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_unexisting_url(self):
        response = self.client.post(
            '/api/v2/auth/lo8563gin', data=json.dumps(user_login), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
        self.assertEqual(result['message'], 'resource not found', msg='not allowed')
    ...
    def test_edit_role_key(self):
        ...
        response = self.client.post(
            ...
            headers=self.get_token())
        response1 = self.client.put(
            '/api/v2/users/1', data=json.dumps(wrong_role_value), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response1.data.decode())
