...
from utils.dummy import edit_office, category_name, office_name_keys, create_office2, office_keys, office_name, \
    name_exists, category_restriction, create_account, user_login
...
class TestOffice(BaseTest):
    ...
    def test_wrong_category_value(self):
        ...
        response = self.client.post(
            '/api/v2/offices', data=json.dumps(category_restriction), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_edit_office_name(self):
        ...
        response = self.client.put(
            '/api/v2/offices/1', data=json.dumps(office_name), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
        self.assertEqual(result['message'], 'The name of the office is in wrong format!')
        assert response.status_code == 400
    ...
    def test_edit_office_category(self):
        ...
        response1 = self.client.post(
            ...
            headers=self.get_token())
        response = self.client.put(
            '/api/v2/offices/1', data=json.dumps(category_name), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_edit_office_keys(self):
        ...
        response = self.client.put(
            '/api/v2/offices/1', data=json.dumps(office_name_keys), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response.data.decode())
    ...
    def test_edit_party(self):
        ...
        response1 = self.client.put(
            '/api/v2/offices/1', data=json.dumps(edit_office), content_type='application/json',
            headers=self.get_token())
        result = json.loads(response1.data.decode())
